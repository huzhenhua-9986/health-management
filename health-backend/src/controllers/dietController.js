...
export async function getList(req, res) {
  try {
    const { meal_type, start_date, end_date, page, page_size } = req.query
    const user_id = req.user.role === 'admin' && req.query.user_id ? req.query.user_id : req.user.id

    const result = await DietData.getList({
      user_id,
      meal_type,
      start_date,
      end_date,
...
export async function create(req, res) {
  try {
    ...
    const record = await DietData.create(data)

    await SystemLog.create({
      user_id: req.user.id,
      action: 'create',
      resource_type: 'diet_data',
...
export async function getOne(req, res) {
  try {
    const record = await DietData.findById(req.params.id)
...
export async function update(req, res) {
  try {
    ...
    const updated = await DietData.update(req.params.id, req.body)
...
export async function remove(req, res) {
  try {
    ...
    await DietData.delete(req.params.id)
